...
class Reranker:
    ...
    def rerank(self, evidences: List[RetrievalEvidence], query: str, top_k: int = 5) -> List[RetrievalEvidence]:
        ...
        for ev in evidences:
            ...
            # 1. genre_match_score：题材精确匹配 (0.3)
            work_genre = work.get("genre", "").lower()
            genre_match_score = 1.0 if work_genre in query.lower() else 0.0
            
            # 2. tag_overlap_score：标签重合度 (0.3)
            work_tags = [t.lower() for t in work.get("tags", [])]
            matched_tags = sum(1 for t in work_tags if t in query.lower())
            tag_overlap_score = matched_tags / len(work_tags) if work_tags else 0.0
            
            # 3. conflict_similarity_score：核心冲突文字相似度 (0.3)
            work_conflict = work.get("core_conflict", "")
            conflict_set = self._tokenize(work_conflict)
            conflict_similarity_score = self._jaccard_similarity(conflict_set, query_set)
            
            # 4. character_setup_score：人物设定文字相似度 (0.1)
            work_char_setup = work.get("character_setup", "")
            char_set = self._tokenize(work_char_setup)
            character_setup_score = self._jaccard_similarity(char_set, query_set)
